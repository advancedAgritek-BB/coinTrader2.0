...
1. Install the dependencies:
   ...
   **Important:** this repository ships with a minimal `ccxt` stub for the unit
   tests. When running the trading bot you must use the real `ccxt` package.
   Delete or rename the local `ccxt` directory (or run the bot from outside the
   repository root) so Python imports the installed library.
