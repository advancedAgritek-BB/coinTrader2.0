...
def test_update_caches_default_limit(monkeypatch):
    ctx = BotContext(
        ...
        df_cache={},
        regime_cache={},
        config={"timeframe": "1h", "timeframe_limits": {"1h": 120}},
    )
    ctx.exchange = object()
    ...
    assert dummy_update.kwargs["timeframe_limits"] == {"1h": 120}
...
def test_update_caches_override(monkeypatch):
    ctx = BotContext(
        ...
        config={
            "timeframe": "1h",
            "cycle_lookback_limit": 50,
            "timeframe_limits": {"1h": 50},
        },
    )
    ctx.exchange = object()
    ctx.current_batch = ["BTC/USDT"]
    ...
    assert dummy_update.kwargs["timeframe_limits"] == {"1h": 50}
